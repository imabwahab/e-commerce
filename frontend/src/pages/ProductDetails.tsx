...
const ProductDetails = () => {
  const { id } = useParams();
  const { filteredProducts, addToCart, wishList, toggleWishList, cartItems } = useContext(AppContext);
  const product: Product | undefined = filteredProducts.find((pro) => pro._id === id);
  ...
  return (
    <>
      <div className='max-w-7xl mx-auto mt-32 w-full'>
        {/* Section Header */}
        <div className=''>
          <div className='flex items-center px-4 md:px-8 gap-3 mb-2'>
            <span className='w-4 h-10 bg-red-500 rounded'></span>
            <p className='text-red-500 font-semibold text-base'>Product Details</p>
          </div>
        </div>
            ...
            <div className="flex-1">
              ...
              <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div className="flex items-baseline gap-3">
                  ...
                  {
                    discount !== 0 && <p className="text-lg text-gray-500 line-through">
                      ${product.price}
                    </p>
                  }
                  {
                    discount !== 0 && <span className="px-2 py-1 bg-green-100 text-green-700 text-sm font-semibold rounded">
                      {discount}% OFF
                    </span>
                  }
                </div>
      ...
      {relatedProducts.length > 0 && (
        <div className="max-w-7xl justify-items-center mx-auto px-6 md:px-8 my-16">
